...
LRESULT CVCMDLDlg::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
...
// Create Side tool panel tabs
	m_ToolTab.AddTab("C", &m_CreateTab, IDD_TABDLG_CREATE);
	m_ToolTab.AddTab("M", &m_ModifyTab, IDD_TABDLG_MODIFY);
	m_ToolTab.AddTab("B", &m_BonesTab, IDD_TABDLG_BONES);
	m_ToolTab.AddTab("V", &m_ViewTab, IDD_TABDLG_VIEW);
...
void CVCMDLDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CVCMDLDlg::OnShowWindow(BOOL bShow, UINT nStatus) 
{
...
void CVCMDLDlg::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
   SetFocus();
         ...
         if (g_CurView == vtCamera)
         {
            ...
            for (int n=0;n<g_CurMdl.Head.num_xyz;n++)
		         if (g_CurMdl.Verts[n].Visible)
   	         	if (fabs((float)g_posx[n]-fx)<3 && fabs((float)g_posy[n]-fy)<3)
                     NewSelect[n] = true;
...
void CVCMDLDlg::MouseDown(CMouseState Button, CShiftState Shift, int X, int Y)
{
   ...
   if ( (Button.m_Right && Shift.m_Ctrl) ||
         (fabs((double)(X-VWidthL - m_SideToolDlg.GetWidth()))<2
		 && fabs((double)(Y-VHeightT<2))))
   {
...
/*			if (CreateBoneBut->Down)
	...
	if (g_MainAction == atBuildFace)
   {
      ...
      if (g_CurView == vtCamera)
      {
         ...
         for (int n=0;n<g_CurMdl.Head.num_xyz;n++)
         if (g_CurMdl.Verts[n].Visible)
            if (fabs((double)g_posx[n]-fx)<3 && fabs((double)g_posy[n]-fy)<3)
            {
...
void CVCMDLDlg::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
void CVCMDLDlg::OnLButtonUp(UINT nFlags, CPoint point) 
{
...
void CVCMDLDlg::OnMButtonDown(UINT nFlags, CPoint point) 
{
// First determine if the point is inside the for viewports or not
   if (point.x < m_SideToolDlg.GetWidth() || point.y>VHeightT+VHeightB)
   {
...
void CVCMDLDlg::OnMButtonUp(UINT nFlags, CPoint point) 
{
...
void CVCMDLDlg::OnRButtonDown(UINT nFlags, CPoint point) 
{
// First determine if the point is inside the for viewports or not
   if (point.x < m_SideToolDlg.GetWidth() || point.y>VHeightT+VHeightB)
   {
...
void CVCMDLDlg::OnRButtonUp(UINT nFlags, CPoint point) 
{
...
void CVCMDLDlg::OnMouseMove(UINT nFlags, CPoint point) 
{
   int   X,Y;
   int   ClientWidth, ClientHeight, PanelWidth;
   RECT  ClientRect;
...
{
   static UINT    LastCursor = NULL;
   UINT     targ = IDC_CURSORARROW;
...
void CVCMDLDlg::OnTimer(UINT nIDEvent) 
{
...
UINT CVCMDLDlg::IsMenuChecked(UINT IDItem)
{
   return GetMenu()->GetMenuState(IDItem, MF_CHECKED) & MF_CHECKED;
}

UINT CVCMDLDlg::ToggleMenuChecked(UINT IDItem)
{
...
void CVCMDLDlg::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
   SetFocus();
   if (nChar == VK_CONTROL)  // control key
   {
